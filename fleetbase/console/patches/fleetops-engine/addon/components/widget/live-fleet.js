...
import { MAPBOX_TILE_URL_LIGHT, MAPBOX_TILE_URL_DARK, MAPBOX_ATTRIBUTION } from '../../utils/mapbox-tiles';

const DEFAULT_CENTER = [1.31, 103.85];
const DEFAULT_ZOOM = 11;
// Match the rest of fleetops' Leaflet maps (Mapbox basemap), which keeps the
...
const TILE_URL = MAPBOX_TILE_URL_LIGHT;
const TILE_URL_DARK = MAPBOX_TILE_URL_DARK;
const TILE_ATTRIBUTION = MAPBOX_ATTRIBUTION;
...
export default class WidgetLiveFleetComponent extends Component {
    ...
    // Applied from company map settings when configured (see #applyCustomTileUrl).
    // Falls back to the default Carto tiles above when not set. Additive only --
    // mirrors the same pattern used by Map::LeafletLiveMap and PositionsReplay,
    // but fetched directly here (rather than injecting the fleetops-engine's
    // local `mapSettings` service) because this widget is rendered under the
    // HOST owner (see comment in constructor below re: LazyEngineComponent) and
    // `map-settings` is an engine-local service not in ember-core's shared
    // `services` list, so `@service mapSettings` would fail to resolve there.
    // `fetch`, `socket`, and `currentUser` above work because those ARE in that
    // shared list. Doesn't depend on Carto's now API-key-gated anonymous tiles.
    @tracked customTileUrl = null;
    @tracked customTileUrlDark = null;

    tileAttribution = TILE_ATTRIBUTION;

    get tileUrl() {
        const isDark = typeof document !== 'undefined' && document.documentElement?.dataset?.theme === 'dark';
        if (isDark && this.customTileUrlDark) {
            return this.customTileUrlDark;
        }
        if (!isDark && this.customTileUrl) {
     ...
     * Falls back to the existing default Carto tileUrl when not set. Additive
     * only -- does not change behavior when no custom URL is saved, and any
     * fetch failure is swallowed so this never blocks the widget from loading.
     */
    #applyCustomTileUrl() {
        this.fetch
            ...
            .then((settings) => {
                ...
                const customTileUrl = settings?.leafletTileUrl;
                if (customTileUrl) {
                    this.customTileUrl = customTileUrl;
                    this.customTileUrlDark = customTileUrl;
                }
            })
            .catch((e) => {
                debug(`[live-fleet] Failed to load map settings: ${e?.message ?? e}`);
            });
    }
