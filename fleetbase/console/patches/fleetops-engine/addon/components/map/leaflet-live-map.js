...
import { MAPBOX_TILE_URL_LIGHT, MAPBOX_TILE_URL_DARK } from '../../utils/mapbox-tiles';

// Leaflet is loaded as a global script in this build (window.L), not as an ES module.
// Importing 'leaflet' here previously broke the whole map route render.
const L = window.L;
...
export default class MapLeafletLiveMapComponent extends Component {
    ...
    @tracked tileUrl = MAPBOX_TILE_URL_LIGHT;
    @tracked theme = 'light';
    @tracked routes = [];
    @tracked drivers = [];
    @tracked vehicles = [];
    ...
    constructor() {
        ...
        // Apply a custom Leaflet tile URL from company map settings, if configured.
        // Falls back to the existing default tileUrl when not set. Additive only --
        // does not change behavior when no custom URL is saved.
        this.mapSettings.load().then(() => {
            if (this.isDestroying || this.isDestroyed) {
                return;
            }
            const customTileUrl = this.mapSettings.leafletTileUrl;
            if (customTileUrl) {
                this.tileUrl = customTileUrl;
            }
        });
    ...
    @action didLoad({ target: map }) {
        ...
        // Additive: start polling Traccar (GeoPulse) live positions -- Leaflet-only, does not affect existing resources
        this.#startTraccarLive(map);
    }
    ...
    #changeTileSource(source) {
        switch (source) {
            case 'dark':
                this.theme = 'dark';
                this.tileUrl = MAPBOX_TILE_URL_DARK;
                break;
            case 'custom':
                this.theme = 'custom';
                this.tileUrl = source.startsWith('https://') ? source : MAPBOX_TILE_URL_LIGHT;
                break;
            case 'light':
            default:
                this.theme = 'light';
                this.tileUrl = MAPBOX_TILE_URL_LIGHT;
                break;
        }
    }

    /**
     * Additive Traccar (GeoPulse) live-position layer.
     * Polls the backend proxy (int/v1/traccar/positions) and renders markers
     * on the existing Leaflet map instance, without touching any other map
     * behaviour. Leaflet-only; a no-op when the Google Maps provider is active.
     */
    #startTraccarLive(map) {
        if (this.shouldUseGoogleMaps || !map || typeof map.addLayer !== 'function') {
            return;
        }

        this.#stopTraccarLive();

        this._traccarLayer = L.layerGroup().addTo(map);
        this.#refreshTraccarPositions();
        this._traccarTimer = setInterval(() => this.#refreshTraccarPositions(), 15000);
    }

    #stopTraccarLive() {
        if (this._traccarTimer) {
            clearInterval(this._traccarTimer);
            this._traccarTimer = null;
        }
        if (this._traccarLayer && this.map && typeof this.map.removeLayer === 'function') {
            this.map.removeLayer(this._traccarLayer);
        }
        this._traccarLayer = null;
    }

    async #refreshTraccarPositions() {
        if (!this._traccarLayer) {
            return;
        }

        try {
            const positions = await this.fetch.get('traccar/positions');
            this._traccarLayer.clearLayers();
            ...
            (positions ?? []).forEach((position) => {
                ...
                const color = this.#traccarColorFor(position.id);
                ...
                const fixTime = position.fixTime ? new Date(position.fixTime).toLocaleString() : '—';
                const name = position.name ?? `Device #${position.id ?? '?'}`;
                const speed = position.speed ?? '—';
                marker.bindPopup(`<strong>${name}</strong><br/>Speed: ${speed} km/h<br/>Time: ${fixTime}`);
                marker.addTo(this._traccarLayer);
            });
        } catch (err) {
            debug('Failed to load Traccar live positions: ' + err.message);
        }
    }
    ...
    #traccarColorFor(id) {
        ...
        for (let i = 0; i < key.length; i++) {
            hash = (hash + key.charCodeAt(i)) % palette.length;
        }
        return palette[hash];
    }
}
