...
export default class PositionPlaybackService extends Service {
    ...
    initialize(options = {}) {
        ...
        // Prefer a marker local to THIS replay session (e.g. the small map embedded
        // in the Positions/Replay tab) over a marker registered in the global
        // MapManager registry. The global registry is shared app-wide (the
        // Fleet-Ops live map registers a marker per vehicle keyed by the same
        // subject id), so checking it first would silently animate a marker on a
        // different, possibly off-screen map instead of the one the user is
        // actually looking at.
        if (existingLeafletMarker) {
            this.markerId = null;
            this.marker = existingLeafletMarker;
        } else if (subjectId && this.mapManager?.hasMarker(subjectId)) {
