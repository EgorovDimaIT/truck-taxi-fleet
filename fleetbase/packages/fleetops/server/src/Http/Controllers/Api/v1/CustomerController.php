...
class CustomerController extends Controller
{
    ...
    public function create(CreateCustomerRequest $request)
    {
        ...
        // Mark the identity verified now that the one-time signup code has been
        // confirmed. The Customer Portal login rejects unverified users, so this
        // must run for both newly created and pre-existing $user rows reached
        // via this signup flow.
        if ($isEmail) {
    ...
    public function createOrder(CreateCustomerOrderRequest $request)
    {
        ...
        $order = $this->createOrderRecord([
            ...
            'customer_type'     => Utils::getMutationType($customer),
