...
class DriverController extends Controller
{
    ...
    public function track(string $id, Request $request)
    {
        ...
        // ----------------------------------------------------------------
        // Smartphone-app connectivity device sync
        //
        // If this driver has a Connectivity > Devices record attached to
        // them (e.g. type smartphone_app_device, linked via
        // attachable_type/attachable_uuid), reflect this location ping
        // there too: mark it online and log a device event so it shows up
        // under Connectivity > Devices > Events. Wrapped defensively so a
        // Connectivity module issue never blocks the driver's location
        // update from succeeding.
        // ----------------------------------------------------------------
        try {
            $connectedDevice = Device::where('attachable_type', Driver::class)
                ->where('attachable_uuid', $driver->uuid)
                ->first();

            if ($connectedDevice) {
                $connectedDevice->updateQuietly([
                    'online'         => true,
                    'last_online_at' => now(),
                    'last_position'  => new Point($latitude, $longitude),
                ]);

                DeviceEvent::create([
                    'company_uuid' => $connectedDevice->company_uuid,
                    'device_uuid'  => $connectedDevice->uuid,
                    'event_type'   => 'location_update',
                    'severity'     => 'info',
                    'message'      => 'Location update received from driver app',
                    'location'     => new Point($latitude, $longitude),
                    'data'         => [
                        'latitude'  => $latitude,
                        'longitude' => $longitude,
        ...
        } catch (\Throwable $deviceSyncException) {
            if (app()->bound('sentry')) {
                app('sentry')->captureException($deviceSyncException);
            }
        }
    ...
    public function registerDevice(Request $request, ?string $id = null)
    {
        try {
            ...
            //
            // NOTE: $request must come FIRST and must NOT have a default value.
            // Laravel's controller dependency resolver only auto-injects a
            // type-hinted parameter from the container when that parameter has
            // no default value; a nullable param with "= null" is treated as
            // already resolved and is left null, which previously caused
            // "Call to a member function input() on null" below.
