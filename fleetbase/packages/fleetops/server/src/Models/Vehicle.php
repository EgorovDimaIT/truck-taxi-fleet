...
class Vehicle extends Model
{
    ...
    public function createPosition(array $attributes = [], Model|string|null $destination = null): ?Position
    {
        ...
        // Allow the actual GPS fix time to be recorded instead of the DB insert
        // time, e.g. for telematics live updates / historical backfills (Traccar, etc).
        // 'created_at' is not in Position::$fillable, so it is never mass-assigned
        // above; we patch it in as a follow-up update, which Eloquent will not
        // clobber, since timestamps are only auto-set for created_at on insert.
        $occurredAt = $attributes['occurred_at'] ?? $attributes['created_at'] ?? null;
        if ($position && $occurredAt) {
            try {
                $position->created_at = $occurredAt instanceof \DateTimeInterface
                    ? $occurredAt
                    : \Illuminate\Support\Carbon::parse($occurredAt);
                $position->save();
            } catch (\Throwable $e) {
                // Malformed timestamp - keep the DB insert time as a safe fallback.
            }
        }
